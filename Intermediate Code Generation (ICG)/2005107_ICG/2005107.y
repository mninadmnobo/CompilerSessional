/*
yacc --yacc -d --debug 2005107.y -o y.tab.cpp
echo 'step-1: y.tab.cpp and y.tab.hpp created'
flex -o 2005107.cpp 2005107.l
echo 'step-2: scanner created'
g++ -w *.cpp
echo 'step-3: a.out created'
rm 2005107.cpp y.tab.cpp y.tab.hpp
./a.out input.c
rm a.out 
...
#include "2005107_AssemblyGenerator.h"
using namespace std;
int yyparse(void);
int yylex(void);
extern FILE *yyin;
...
compound_statement : LCURL statements M RCURL
    {
        $$ =new ParseTree(false,"compound_statement : LCURL statements M RCURL",$1->getLineNo(),$4->getLineNo());
        ParseTree* terminalLCURL= new ParseTree(true,$1->getType()+" : "+$1->getName(),$1->getLineNo(),$1->getLineNo());
        ParseTree* terminalRCURL= new ParseTree(true,$4->getType()+" : "+$4->getName(),$4->getLineNo(),$4->getLineNo());
        $$->addChild(terminalLCURL)->addChild($2)->addChild($3)->addChild(terminalRCURL);
...
declaration_list : declaration_list COMMA ID
 	...
 	| declaration_list COMMA ID LSQUARE CONST_INT RSQUARE
    {
        ...
        symbol->setLineNo($3->getLineNo());
        symbol->setArrayOffset(atoi($5->getName().c_str()));
    }
 	| ID
    {
 	...
 	| ID LSQUARE CONST_INT RSQUARE
    {
        ...
        SymbolInfo* symbol=new SymbolInfo($1->getName(),$1->getType());
        terminalID->setVarType($1->getName());
        current_dec_list->push_back(symbol);
        symbol->setSymbolType("ARRAY");
        symbol->setLineNo($1->getLineNo());
        symbol->setArrayOffset(atoi($3->getName().c_str()));
    }
 	;	  
statements : statement
    ...
    {
        $$=new ParseTree(false,"statements : statements M statement",$1->getLineBegin(),$3->getLineEnd());
...
statement : var_declaration
    ...
    {
        $$=new ParseTree(false,"statement : FOR LPAREN expression_statement M expression_statement M expression N RPAREN M statement N",$3->getLineBegin(),$11->getLineEnd());
        ...
        ParseTree* terminalRPAREN= new ParseTree(true,$9->getType()+" : "+$9->getName(),$9->getLineNo(),$9->getLineNo());
        $$->addChild(terminalFOR)->addChild(terminalLPAREN)->addChild($3)->addChild($4)->addChild($5)->addChild($6)->addChild($7)->addChild($8)->addChild(terminalRPAREN)->addChild($10)->addChild($11)->addChild($12);        
    ...
    {
        $$=new ParseTree(false,"statement : IF LPAREN expression RPAREN M statement-",$1->getLineNo(),$6->getLineEnd());
        ParseTree* terminalIF= new ParseTree(true,$1->getType()+" : "+$1->getName(),$1->getLineNo(),$1->getLineNo());
        ParseTree* terminalLPAREN= new ParseTree(true,$2->getType()+" : "+$2->getName(),$2->getLineNo(),$2->getLineNo());
        ParseTree* terminalRPAREN= new ParseTree(true,$4->getType()+" : "+$4->getName(),$4->getLineNo(),$4->getLineNo());
        $$->addChild(terminalIF)->addChild(terminalLPAREN)->addChild($3)->addChild(terminalRPAREN)->addChild($5)->addChild($6);
        log_file<<"statement : IF LPAREN expression RPAREN statement "<<endl;
    }
	| IF LPAREN expression RPAREN M statement ELSE N M statement 
    {
        $$=new ParseTree(false,"statement : IF LPAREN expression RPAREN M statement ELSE N M statement",$3->getLineBegin(),$10->getLineEnd());
        ParseTree* terminalIF= new ParseTree(true,$1->getType()+" : "+$1->getName(),$1->getLineNo(),$1->getLineNo());
        ParseTree* terminalLPAREN= new ParseTree(true,$2->getType()+" : "+$2->getName(),$2->getLineNo(),$2->getLineNo());
        ParseTree* terminalRPAREN= new ParseTree(true,$4->getType()+" : "+$4->getName(),$4->getLineNo(),$4->getLineNo());
        ParseTree* terminalELSE= new ParseTree(true,$7->getType()+" : "+$7->getName(),$7->getLineNo(),$7->getLineNo());
        $$->addChild(terminalIF)->addChild(terminalLPAREN)->addChild($3)->addChild(terminalRPAREN)->addChild($5)->addChild($6)->addChild(terminalELSE)->addChild($8)->addChild($9)->addChild($10);
        log_file<<"statement : IF LPAREN expression RPAREN statement ELSE statement "<<endl;
    }
	| WHILE M LPAREN expression RPAREN M statement
    {
        $$=new ParseTree(false,"statement : WHILE M LPAREN expression RPAREN M statement",$1->getLineNo(),$7->getLineEnd());
        ParseTree* terminalWHILE= new ParseTree(true,$1->getType()+" : "+$1->getName(),$1->getLineNo(),$1->getLineNo());
        ParseTree* terminalLPAREN= new ParseTree(true,$3->getType()+" : "+$3->getName(),$3->getLineNo(),$3->getLineNo());
        ParseTree* terminalRPAREN= new ParseTree(true,$5->getType()+" : "+$5->getName(),$5->getLineNo(),$5->getLineNo());
        $$->addChild(terminalWHILE)->addChild($2)->addChild(terminalLPAREN)->addChild($4)->addChild(terminalRPAREN)->addChild($6)->addChild($7);
	...
	| RETURN expression SEMICOLON N
    {
        $$=new ParseTree(false,"statement : RETURN expression SEMICOLON N",$1->getLineNo(),$3->getLineNo());
        ParseTree* terminalRETURN= new ParseTree(true,$1->getType()+" : "+$1->getName(),$1->getLineNo(),$1->getLineNo());
        ParseTree* terminalSEMICOLON= new ParseTree(true,$3->getType()+" : "+$3->getName(),$3->getLineNo(),$3->getLineNo());
        $$->addChild(terminalRETURN)->addChild($2)->addChild(terminalSEMICOLON)->addChild($4);
...
logic_expression : rel_expression 
    ...
    {
        $$=new ParseTree(false,"logic_expression : rel_expression LOGICOP M rel_expression",$1->getLineBegin(),$4->getLineEnd());
        ParseTree* terminalLOGICOP= new ParseTree(true,$2->getType()+" : "+$2->getName(),$2->getLineNo(),$2->getLineNo());
        $$->addChild($1)->addChild(terminalLOGICOP)->addChild($3)->addChild($4);
        log_file<<"logic_expression : rel_expression LOGICOP rel_expression 	 	 "<<endl;

        string type = h->checkType($1->getExp().dataType,$4->getExp().dataType);
        if(type!="INT")
        {
...
unary_expression : ADDOP unary_expression  
    {
        ...
        $$->addChild(terminalADDOP)->addChild($2);
...
M : 
    {
        $$=new ParseTree(true,"backpatch",0,0);
        log_file<<"M : "<<endl;
    }
N :
    {
        $$=new ParseTree(true,"jump rule",0,0);
        log_file<<"jump rule : "<<endl;
    }
    ;
...
int main(int argc,char *argv[])
{
    ...
    //assembly file opening
   // assembly_file.open("assembly.txt");
	yyin=fp;
	yyparse();
